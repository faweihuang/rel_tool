...
CREATE TABLE `rel_role` (
  ...
  `grants` text NOT NULL DEFAULT '' COMMENT '权限内容, api表ID数组string',
  `create_date` int NOT NULL DEFAULT '0' COMMENT '创建时间',
  `status` tinyint NOT NULL DEFAULT '0' COMMENT '状态: 0 禁用, 1 正常',
  `is_delete` tinyint NOT NULL DEFAULT '0' COMMENT '是否删除: 0 正常， 1 已删除',
  PRIMARY KEY (`id`)
...
CREATE TABLE `rel_project`(
   ...
   `exclude`               text NOT NULL DEFAULT '' COMMENT '排除同步文件',
   `create_date`      int NOT NULL DEFAULT '0' COMMENT '创建时间',
   `status`                  tinyint NOT NULL DEFAULT '0' COMMENT '状态: 0 禁用, 1 正常',
...
CREATE TABLE `rel_nodes`(
   ...
   `nodes`         text NOT NULL DEFAULT '' COMMENT '服务器节点列表',
...
CREATE TABLE `rel_tasks`(
    ...
    `val`           text NOT NULL DEFAULT '' COMMENT '内容',
...
CREATE TABLE `rel_running`(
    ...
    `task_data`     MediumText NOT NULL DEFAULT '' COMMENT "任务所需数据",
    `result`        MediumText NOT NULL DEFAULT '' COMMENT "执行反馈",
...
CREATE TABLE `rel_logs`(
   ...
   `val`           MEDIUMTEXT NOT NULL DEFAULT '' COMMENT '日志内容',
